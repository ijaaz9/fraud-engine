...
class FraudFlagControllerTest {
    ...
    void getFlags_exceedingMaxPageSize_capsAtHundred() throws Exception {
        PagedFlagResponse response = PagedFlagResponse.builder()
                .content(List.of()).page(0).size(100).totalElements(0).totalPages(0).last(true)
                .build();
    ...
    // -------------------------------------------------------------------------
    // GET /api/v1/flags/{transactionId}
    // -------------------------------------------------------------------------

    @Test
    @DisplayName("GET /api/v1/flags/{transactionId} — returns 200 with flag detail")
    void getFlagsForTransaction_returnsDetail() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/flags/txn-001")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("GET /api/v1/flags/{transactionId} — returns 404 when transaction not found")
    void getFlagsForTransaction_notFound_returns404() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/flags/txn-unknown")
                        .accept(MediaType.APPLICATION_JSON))
